...
class HiveMind{
    ...
    init(frame : NSRect, numBoids : Int){
        ...
        populateBoids(numBoids: numBoids)
    ...
    private func initMtlPipeline(){
        ...
        let buffSize = MemoryLayout<BoidStruct>.stride * boids.count
        buffer = device.makeBuffer(length: buffSize, options: .storageModeShared)
    }
    ...
    func updateBoidBuffer(device: MTLDevice){
        let boidStructs : [BoidStruct] = boids.map{boidToGpu(boid: $0)}

        let bufferPointer = buffer.contents().assumingMemoryBound(to: BoidStruct.self)

        for i in 0..<boids.count {
            bufferPointer[i] = boidStructs[i]
        }
    }
...
func flock(){
    ...
    updateBoidBuffer(device: device)
    ...
    let boidPointer = buffer.contents().bindMemory(to: BoidStruct.self, capacity: boids.count)
